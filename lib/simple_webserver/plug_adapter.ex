defmodule SimpleWebserver.PlugAdapter do
  def dispatch(request, plug) do
    %{headers: headers, full_path: full_path} = SimpleWebserver.read_request(request) |> IO.inspect()

    %Plug.Conn{
      adapter: {SimpleWebserver.PlugAdapter, request},
      owner: self(),
  ...
  def send_resp(socket, status, headers, body) do
    ...
    SimpleWebserver.send_response(socket, response)
    {:ok, nil, socket}
  end

  def child_spec(plug: plug, port: port) do
    SimpleWebserver.child_spec(port: port, dispatch: &dispatch(&1, plug))
  end
